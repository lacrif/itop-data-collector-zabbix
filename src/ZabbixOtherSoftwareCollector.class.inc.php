...
class ZabbixOtherSoftwareCollector extends ZabbixCsvCollector
{
	protected $idx;
		...
		if ($bSucceed){
			...
			foreach( $aResults['result'] as $aResult ) 
			{
				...
				foreach( $aLastValues as $aLastValue ) {
					...
					$_sIndex = $sHost."::".$aLastValue->Vendor."::".$aLastValue->Name."::".$aLastValue->Version;
					...
					$aData = array(
						...
						"system_id" => $sHost,
						"name" => $aLastValue->Vendor." ".$aLastValue->Name." ".$aLastValue->Version,
						"software_id" => $aLastValue->Name." ".$aLastValue->Version
					);
